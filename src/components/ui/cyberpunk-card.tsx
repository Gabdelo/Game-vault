      ...
      setColumns((prevColumns) =>
        prevColumns.map((column) => {
          const updatedCharacters = column.characters.map((char, i) => ({
            ...char,
            char:
              Math.random() < 0.1 ? matrixChars[Math.floor(Math.random() * matrixChars.length)] : char.char,
...
export const CyberpunkCard = ({
  ...
  return (
    ...
    >
      ...
      {isHovered && <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent pointer-events-none" />}
    </div>
  )
}
