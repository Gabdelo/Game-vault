...
import { getOtherLibrary, getOtherProfile } from "@/services/profileService"
...
export const UsersProfile = () => {
    ...
    const { id } = useParams() // userId del amigo
    ...
    usePageTitle(profile?.username ? `${profile.username}` : "Perfil de Usuario") 
    ...
    // Calcular URL del avatar
    useEffect(() => {
        if (profile?.avatar) {
            const DIRECTUS_URL = import.meta.env.VITE_RAWG_DIRECTUS_URL || "https://directus-latest-i2px.onrender.com"
            const url = `${DIRECTUS_URL}/assets/${profile.avatar}`
            setAvatarUrl(url)
        }
    }, [profile?.avatar])

    // Resetear página cuando cambia la biblioteca
    useEffect(() => {
        setCurrentPage(1)
    }, [library])
    ...
    const stats = {
     ...
     // Debug: Verificar stats en consola
                ...
                {profile?.is_public ? (
                    <>
                                                    ...
                                                    <div className="relative w-full aspect-[3/4] overflow-hidden">
                                                        {game.background_image ? (
                                                            ...
                                                            <div className="w-full h-full bg-gray-600 flex items-center justify-center">
                                                                <span className="text-gray-400 text-xs text-center px-2">{game.name}</span>
                                                            </div>
                                                        )}
                                                        {/* Status badge */}
                                                            {game.status && (
                        ...
                        <div className="bg-gray-800/60 border border-yellow-400/20 p-6">
                            ...
                            {library.length === 0 ? (
                                <p className="text-gray-400 text-center py-8">Esta biblioteca está vacía</p>
                            ) : (
                                <>
                                    {/* Games Grid */}
                    ...
                    <div className="bg-gradient-to-r  p-8 text-center">
                        <div>
                            <FiLock size={48} className="mx-auto mb-4 text-cy" />
                            <p className="text-cy text-lg font-semibold mb-2">Perfil Privado</p>
                        </div>
                        <p className="text-gray-400">Este usuario tiene su perfil como privado. No puedes ver su biblioteca, estadísticas ni información detallada.</p>
                    </div>
                )}
            </div>
        </div>
